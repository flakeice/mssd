...
class TrackHead(torch.nn.Module):
    ...
    def forward_inference(self, features):
        track_boxes = None
        if self.track_memory is None:
            self.track_pool.reset()
        else:
            (template_features, sr, template_boxes) = self.track_memory
    ...
    def flush_track_memory(self, features, tracks):
        assert (len(tracks) == 1)
...
def build_track_head(cfg, track_utils, track_pool):
    ...
    from .EMM.target_sampler import EMMTargetSampler

    tracker = EMM(cfg, track_utils)

    tracker_sampler = EMMTargetSampler(cfg, track_utils)
